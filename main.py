...
def format_currency(sheet_id, row):
    return {
        "repeatCell": {
            "range": {
                "sheetId": sheet_id,
...
def format_number(sheet_id, row):
    return {
        "repeatCell": {
            "range": {
                "sheetId": sheet_id,
...
def format_percent(sheet_id, row):
    return {
        "repeatCell": {
            "range": {
                "sheetId": sheet_id,
...
def format_kpi_color(sheet_id, row, color):
    return {
        "repeatCell": {
            "range": {
                "sheetId": sheet_id,
...
def clear_demo_rows(worksheet, id_column_index, demo_prefix):
    """
...
def generate_demo_business():
    print("\n🟠 Developer Simulator")
    print("Creating demo business data...")

    settlements = []

    for i in range(demo_orders):
        ...
        amount_settled = round(
            payment_amount - service_charge - service_tax,
            2
        )
        ...
        # Generate a random settlement date within the last N days
        random_days = random.randint(0, demo_history_days - 1)
...
demo_orders = int(config_sheet.acell("B3").value)
demo_history_days = int(config_sheet.acell("B4").value)
developer_mode_value = config_sheet.acell("B2").value
...
#existing_payment_records = payments_sheet.get_all_values()

existing_payment_ids = set()

#for row in existing_payment_records[1:]:
    #if len(row) > 1:
        #existing_payment_ids.add(row[1])   # Transaction ID
...
if DEVELOPMENT_MODE:
    ...
    settlements = generate_demo_business()
...
daily_refund_rate = (
    (daily_refunds / daily_orders) * 100
    if daily_orders else 0
)

weekly_refund_rate = (
    (weekly_refunds / weekly_orders) * 100
    if weekly_orders else 0
)

monthly_refund_rate = (
    (monthly_refunds / monthly_orders) * 100
    if monthly_orders else 0
)

total_payments = daily_payments + weekly_payments + monthly_payments
total_refunds = daily_refunds + weekly_refunds + monthly_refunds

overall_transactions = total_payments + total_refunds

#print("Settlement Count:", len(settlements))
for settlement in settlements:
    ...
    daily_refund_rate = round(
        (daily_refunds / max(daily_payments + daily_refunds, 1)) * 100,
        2
    )

    weekly_refund_rate = round(
        (weekly_refunds / max(weekly_payments + weekly_refunds, 1)) * 100,
        2
    )

    monthly_refund_rate = round(
        (monthly_refunds / max(monthly_payments + monthly_refunds, 1)) * 100,
        2
    )

    daily_success_rate = round(
        (daily_payments / max(daily_payments + daily_failed_payments, 1)) * 100,
        2
    )

    weekly_success_rate = round(
        (weekly_payments / max(weekly_payments + weekly_failed_payments, 1)) * 100,
        2
    )

    monthly_success_rate = round(
        (monthly_payments / max(monthly_payments + monthly_failed_payments, 1)) * 100,
        2
    )
    ...
    payment_row = [
        settlement.get("settlement_date"),  # Date & Time
        f"TXN{settlement_id}",  # Transaction ID
        payment_amount,  # Transaction Amount
        random.choice(["UPI", "Card", "Net Banking"]),  # Payment Method
        "SUCCESS",  # Status
        f"9{random.randint(100000000, 999999999)}",  # Phone
        f"ORDER{settlement_id}",  # Order ID
        payment_amount,  # Order Amount
        f"CUST{random.randint(1000, 9999)}"  # Customer Ref
    ]
...
#print(f"{len(payment_rows)} new payments added.")

#print("Payments List:", payment_amounts)
...
print("=" * 40)
print("Settlement Completion =", settlement_completion)
print("=" * 40)
...
print("Completion Color =", completion_color)
print("=" * 40)
...
format_kpi_color(
        ...
        format_kpi_color(
            dashboard_sheet.id,
            MONTHLY_SUCCESS_RATE_ROW,
            success_rate_color
        ),

        format_kpi_color(
            dashboard_sheet.id,
            MONTHLY_REFUND_RATE_ROW,
            refund_rate_color
        ),
...
},
{
    "repeatCell": {
        ...
        format_number(dashboard_sheet.id, DAILY_ORDERS_ROW),  # Daily Orders
        format_number(dashboard_sheet.id, WEEKLY_ORDERS_ROW),  # Weekly Orders
        format_number(dashboard_sheet.id, MONTHLY_ORDERS_ROW),  # Monthly Orders
...
#else:
    #print("ℹ️ No new settlements found.")
print("\n==============================")
print("SYNC SUMMARY")
print("==============================")
